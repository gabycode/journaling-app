...
export async function apiRegister(email: string, contrasena: string) {
  const data = await request<{ mensaje: string; id: number }>('/auth/register', {
    method: 'POST',
    body: JSON.stringify({ email, contrasena }),
  });
  return data;
...
export async function apiCreateEntrada(titulo: string, nota: string, es_publica: boolean) {
...
export async function apiUpdateEntrada(id: string, titulo: string, nota: string, es_publica: boolean) {
