...
export function PublicEntryView() {
  ...
  useEffect(() => {
    ...
    apiGetEntradaPublica(id)
      .then(data => { setEntry(data); setStatus('ok'); })
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#FAFAF8', fontFamily: sans }}>
      ...
      <article style={{ maxWidth: '680px', margin: '0 auto', padding: '64px 24px 96px' }}>
        <h1 style={{ fontFamily: serif, fontSize: 'clamp(30px, 5vw, 44px)', fontWeight: '400', color: '#1A1A18', lineHeight: '1.2', margin: '0 0 20px', letterSpacing: '-0.5px' }}>
        ...
        <div>
          {paragraphs.length > 0 ? paragraphs.map((para, i) => (
            <p key={i} style={{ fontFamily: sans, fontSize: '17px', color: '#1A1A18', lineHeight: '1.85', margin: '0 0 24px' }}>
              {para}
            </p>
          )) : (
