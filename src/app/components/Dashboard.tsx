...
import { apiGetEntradasPublicas } from "../api";
import {
  BookOpen,
  PenLine,
  Globe,
...
export function Dashboard() {
  ...
  const [publicEntries, setPublicEntries] = useState<DashboardEntry[]>([]);
  const [publicLoading, setPublicLoading] = useState(false);
  const [publicError, setPublicError] = useState("");

  const rawFilter = searchParams.get("filter") as FilterType | null;
  const filter: FilterType =
    rawFilter === "private" || rawFilter === "public" ? rawFilter : "all";
  const isGlobalPublicRoute = location.pathname === "/public-entries";
  ...
  useEffect(() => {
    if (!user || !isGlobalPublicRoute) return;

    setPublicError("");
    setPublicLoading(true);
    apiGetEntradasPublicas()
      .then((data) => {
        const mapped: DashboardEntry[] = data.map((e) => ({
          ...
          updatedAt: e.fecha,
          isPublic: true,
          userId: e.usuario_id ? String(e.usuario_id) : "public",
          authorName: e.autor || e.nombre_usuario,
          canManage: e.usuario_id ? String(e.usuario_id) === user.id : false,
        }));
        setPublicEntries(mapped);
      })
      .catch((err) => {
        setPublicEntries([]);
        setPublicError(
          err instanceof Error
            ? err.message
            : "No se pudieron cargar las entradas públicas.",
        );
      })
      .finally(() => setPublicLoading(false));
  }, [user, isGlobalPublicRoute]);

  if (!user) return null;

  const filtered: DashboardEntry[] = entries
    .filter((e) =>
      filter === "all" ? true : filter === "private" ? !e.isPublic : e.isPublic,
    )
    .map((e) => ({ ...e, canManage: true }))
    .sort(
      (a, b) =>
        new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime(),
    );

  const visibleEntries = isGlobalPublicRoute ? publicEntries : filtered;
  const isListLoading = isGlobalPublicRoute ? publicLoading : loading;
  ...
  const navItems = [
    {
      ...
      active: !isGlobalPublicRoute && filter === "all" && !showSettings,
    },
    {
      label: "Nueva entrada",
      icon: PenLine,
    ...
    {
      label: "Entradas públicas",
      icon: Globe,
      action: () => {
        setShowSettings(false);
        navigate("/public-entries");
        setMobileMenuOpen(false);
      },
      active: isGlobalPublicRoute,
    },
    {
      label: "Configuración",
      icon: Settings,
  ...
  const pageTitle = showSettings
    ...
    : isGlobalPublicRoute
      ? "Entradas públicas"
      : filter === "public"
        ? "Mis entradas públicas"
        : filter === "private"
          ? "Mis entradas privadas"
          : "Mis entradas";
    ...
    >
      ...
      <aside
        style={{
          ...
          borderRight: "0.5px solid rgba(26,26,24,0.1)",
          ...
          >
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: "12px", color: "#9B9B95" }}>
                  {user.email}
                </div>
              </div>
            </div>
          ...
          {showSettings ? (
              ...
              >
                ...
                <button
                  ...
                  style={{
                    ...
                    gap: "8px",
                    background: "none",
                    border: "0.5px solid rgba(186,90,60,0.25)",
                    borderRadius: "8px",
                    padding: "9px 16px",
                    cursor: "pointer",
                    fontSize: "13px",
            ...
            <div>
                  ...
                  {(
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border:
                          filter === f
                            ? "0.5px solid rgba(26,26,24,0.4)"
                            : "0.5px solid rgba(26,26,24,0.15)",
                        backgroundColor:
                          filter === f ? "#1A1A18" : "transparent",
                    ...
                    >
                      {label} <span style={{ opacity: 0.65 }}>·</span>{" "}
                      {filterCounts[f]}
                    </button>
                  ))}
                </div>
              ...
              {isListLoading ? (
                ...
                >
                  <p style={{ fontSize: "14px", fontFamily: sans }}>
                    Cargando entradas…
                  </p>
                </div>
              ) : isGlobalPublicRoute && publicError ? (
                <div
                  style={{
                    textAlign: "center",
                    padding: "64px 24px",
                ...
                >
                  ...
                  >
                    {isGlobalPublicRoute
                      ? "Aún no hay entradas públicas en la plataforma."
                      : filter === "public"
                        ? "Aún no tienes entradas públicas."
                        : filter === "private"
                          ? "No hay entradas privadas."
                          : "Empieza escribiendo tu primera entrada."}
                  </p>
                  <button
                    onClick={() => navigate("/editor/new")}
                    style={{
...
function EntryCard({
  ...
  const authorLabel =
    entry.authorName || (canManage ? currentUserName : "Autor desconocido");
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {authorLabel} · {fmtDateFull(entry.updatedAt)}
          </span>
        </div>
            ...
            {isConfirmingDelete ? (
              ...
              >
                ...
                <button
                  ...
                  style={iconBtnStyle("#EEF6F1", "#2D6A4F")}
                  title="Confirmar"
                >
